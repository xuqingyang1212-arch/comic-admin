...
import { Search, RotateCcw, ChevronDown, ChevronLeft, ChevronRight, Calendar, X, ZoomIn } from "lucide-react"
...
function FilterInput({
  label, placeholder, value, onChange, width = "w-[150px]",
}: {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <input
        ...
        className={cn(
          "h-[30px] rounded-[6px] border border-[#d1d5db] bg-white px-3 text-[13px] text-[#374151] placeholder-[#9ca3af] outline-none focus:border-[#38c08f] transition-colors",
          width
        )}
      />
    </div>
...
function SelectFilter({
  label, options, value, onChange, width = "w-[120px]",
}: {
  label: string; options: { label: string; value: string }[]; value: string; onChange: (v: string) => void; width?: string
}) {
...
function MonthPanel({ year, month, startDate, endDate, hoverDate, onDayClick, onDayHover }: {
  year: number; month: number; startDate: string; endDate: string; hoverDate: string
  ...
  function inRange(d: number) {
    const s = startDate, e = endDate || hoverDate, f = fmt(d)
    if (!s) return false
    const [lo, hi] = s <= (e || s) ? [s, e || s] : [e || s, s]
    return f > lo && f < hi
  }
  function isStart(d: number) { return fmt(d) === startDate }
  function isEnd(d: number) {
    ...
    return fmt(d) === hi && lo !== hi
  }

  return (
    <div className="w-[200px]">
      <p className="mb-2 text-center text-[13px] font-medium text-[#374151]">{year}年{month}月</p>
      <div className="grid grid-cols-7 gap-y-0.5">
        {DAYS.map((d) => (
          <div key={d} className="flex h-7 items-center justify-center text-[11px] text-[#9ca3af]">{d}</div>
        ))}
        {cells.map((d, i) =>
          d === null ? <div key={`e-${i}`} /> : (
            <button
              key={d}
              onClick={() => onDayClick(fmt(d))}
              onMouseEnter={() => onDayHover(fmt(d))}
              className={cn(
                "flex h-7 w-full items-center justify-center rounded-[4px] text-[12px] transition-colors",
                isStart(d) || isEnd(d)
...
function DateRangePicker({ value, onChange }: { value: string[]; onChange: (v: string[]) => void }) {
  ...
  function prevMonth() {
    if (leftMonth === 1) { setLeftYear((y) => y - 1); setLeftMonth(12) }
    else setLeftMonth((m) => m - 1)
  }
  function nextMonth() {
    if (leftMonth === 12) { setLeftYear((y) => y + 1); setLeftMonth(1) }
    else setLeftMonth((m) => m + 1)
  }
  ...
  function handleDayClick(d: string) {
    if (!startDate || (startDate && endDate)) {
      ...
      const [lo, hi] = d >= startDate ? [startDate, d] : [d, startDate]
      onChange([lo, hi])
      setOpen(false)
    }
  }
  ...
  const displayText = startDate && endDate ? `${startDate} ~ ${endDate}` : startDate ? `${startDate} ~` : ""
      ...
      {open && (
        <div className="absolute left-0 top-[36px] z-50 flex gap-4 rounded-[8px] border border-[#e5e7eb] bg-white px-5 py-4 shadow-lg">
          <div className="flex flex-col">
            ...
            <MonthPanel year={leftYear} month={leftMonth} startDate={startDate} endDate={endDate} hoverDate={hoverDate} onDayClick={handleDayClick} onDayHover={setHoverDate} />
          </div>
          ...
          <div className="flex flex-col">
            ...
            <MonthPanel year={rightYear} month={rightMonth} startDate={startDate} endDate={endDate} hoverDate={hoverDate} onDayClick={handleDayClick} onDayHover={setHoverDate} />
          </div>
        </div>
      )}
    </div>
